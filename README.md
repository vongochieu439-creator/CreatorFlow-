...
页面中的“AI渲染”会请求本地 `/api/generate`。如果 API 未配置或调用失败，系统会保留本地基础稿。内容质检会请求 `/api/quality`，该评分只使用真实 AI 返回结果，不提供模拟分。
...
当前版本提供本地基础稿生成，并通过 `server.js` 接入 OpenAI-compatible 大模型完成 AI 渲染和真实 AI 质检：
