...
- 真实 AI 质检评分：通过本地代理调用大模型，从标题吸引力、结构完整度、平台匹配度、标签覆盖度和风险控制给出综合分与优化建议；未配置 API 时不展示模拟分。
...
页面中的“一键智能优化”和“优化当前平台”会请求本地 `/api/generate`。如果 API 未配置或调用失败，系统会保留本地基础稿。内容质检会请求 `/api/quality`，该评分只使用真实 AI 返回结果，不提供模拟分。
